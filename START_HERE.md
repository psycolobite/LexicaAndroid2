# 🚀 START HERE - Bienvenue dans LexicaAndroid2!

Bienvenue! Si tu viens de rejoindre ce projet, lis ce fichier en premier.

---

## 👋 **En 30 secondes:**

LexicaAndroid2 est une **application mobile Android** (Kotlin) de **flashcards avec mini-jeux** pour apprendre 7000+ mots français.

Pour connaître l'état précis du projet (ce qui est fait, ce qui reste à faire), consultez :
👉 **[FEATURES.md](FEATURES.md)**

---

## 👨‍💼 **Je suis Chef d'Orchestre**

### Version courte
Lis seulement :

1. **[FEATURES.md](FEATURES.md)**
2. **[docs/guides/CONSIGNES_TACHES.md](docs/guides/CONSIGNES_TACHES.md)**
3. **[docs/guides/GUIDE_CHEF_DORCHESTRE.md](docs/guides/GUIDE_CHEF_DORCHESTRE.md)**

### Règles simples
- `main` = branche stable
- chaque tâche = une branche dédiée (`task/TACHE_XX-nom`)
- le Chef d'Orchestre valide puis intègre au bon moment dans `main`
- `DAILY_STANDUP.md` = seul fichier de suivi quotidien
- pas de nouvelle doc si un fichier existant suffit

---

## 👨‍💻 **Je suis Agent Développeur**

### Version courte
Lis seulement :

1. **[FEATURES.md](FEATURES.md)**
2. **[docs/guides/CONSIGNES_TACHES.md](docs/guides/CONSIGNES_TACHES.md)**
3. ta tâche assignée dans ce fichier

### Règles simples
- tu travailles sur **ta branche** (`agent/TACHE_XX-nom`)
- tu travailles dans **ton package** seulement
- pas de build local
- pas de modification des fichiers coeur sans demande d'intégration
- si blocage global : documenter dans `integration_pending/`

---

## 🎮 **Je veux voir les Mini-Jeux**

→ Voir **[FEATURES.md](FEATURES.md)** section "MINI-JEUX"

---

## 🐛 **J'ai une erreur**

→ Aller à **[docs/guides/TROUBLESHOOTING.md](docs/guides/TROUBLESHOOTING.md)**

---

## 📚 **Documentation Complète**

→ Aller à **[docs/INDEX_DOCUMENTS.md](docs/INDEX_DOCUMENTS.md)**

---

## 💬 Questions Fréquentes

### "Où est le code à intégrer ?"
→ Vérifiez le dossier `integration_pending/`.

### "Je dois installer quoi?"
→ Agent : [docs/guides/SETUP_AGENTS_PARALLEL.md](docs/guides/SETUP_AGENTS_PARALLEL.md)
→ Chef : Android Studio suffit.

### "Qui répond à mes questions?"
→ [docs/guides/TROUBLESHOOTING.md](docs/guides/TROUBLESHOOTING.md)

### "Comment contribuer?"
→ [CONTRIBUTING.md](CONTRIBUTING.md)

### "Où est l'historique complet ?"
→ [CHANGELOG.md](CHANGELOG.md)

---

## ✅ Checklist (Première Fois)

- [ ] Lire ce fichier
- [ ] Ouvrir la bonne branche
- [ ] Lire `FEATURES.md`
- [ ] Lire la doc minimale de ton rôle
- [ ] Commencer

---

**Bienvenue dans l'équipe! 🎉**

*Voir [DAILY_STANDUP.md](DAILY_STANDUP.md) pour le journal quotidien.*
