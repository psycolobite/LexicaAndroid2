...
def main():
    ...
    report_lines = []
    report_lines.append("# Rapport Amélioré de Vocabulaire Littéraire (C1)")
    report_lines.append(f"Filtre appliqué : Ratio Littéraire (Livres / Films) > 2.0 (plus inclusif que le ratio initial > 4.0)")
    ...
    def process_static_tier(name, lst):
        ...
        if len(lst) > 30:
            remaining = [w['word'] for w in lst[30:]]
            lines.append("\n*Autres mots de cette catégorie :* " + ", ".join(remaining))
        lines.append("\n")
        return lines
    ...
    report_lines.append("Cette méthode permet d'extraire des milliers de mots littéraires en se passant de Wiktionnaire.")
    ...
    def process_global_tier(name, lst):
        lines = []
        lines.append(f"### Échantillon de la catégorie {name} (Top 50 par Ratio Littéraire)")
    ...
    output_filename = "resultat_mots_test_c1_ameliore.md"
    with open(output_filename, "w", encoding="utf-8") as out:
        out.write("\n".join(report_lines))
        
    print(f"Enhanced report written to {output_filename}")
