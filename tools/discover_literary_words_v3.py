...
==============================
Algorithme V3 - Définitif, calibré sur les statistiques réelles des mots de référence.

Enseignements des analyses V1 et V2 :
  - 94% des mots de référence ont Zipf_books ∈ [1.84, 4.0]
  - 6% ont ratio livres/films < 1.0 (obsolète, codex, gredin...) = bons mots quand même
  - Le ratio n'est PAS un critère discriminant fort pour les mots littéraires
  - 13 mots de référence ont zb=0 dans Lexique383 (procrastination, résilience, solipsisme...)
    Ces mots sont des "faux absents" de Lexique (trop récents ou trop rares pour figurer)
  - Seuls 3 mots de référence sont vraiment "trop communs" : jadis (4.72), chagrin (4.59), brume (4.55)

Logique V3 :
  - Critère PRIMAIRE : Zipf_books ∈ [1.5, 4.3] → score de fréquence
  - Critère SECONDAIRE (bonus) : catégorie Wiktionnaire (soutenu, littéraire, poétique)
  - Critère TERTIAIRE (bonus modeste) : ratio livres/films (mais ne pénalise pas si ratio < 1)
  - Filtre FINAL : éliminer les mots dont le sens est trop courant/familier (liste noire)
  
  Mots avec zb=0 dans Lexique ET dans catégorie Wiktionnaire → score intermédiaire valide
  Mots avec zb=0 dans Lexique ET PAS dans Wiktionnaire → score faible (données insuffisantes)
"""
